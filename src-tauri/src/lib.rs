...
/// Check if the sidecar server is ready by polling the healthcheck endpoint
...
pub async fn start_sidecar(
    ...
    // Set the working directory to resource_dir so the server can find dist/client
    ...
    // Spawn a task to forward sidecar output to tracing
