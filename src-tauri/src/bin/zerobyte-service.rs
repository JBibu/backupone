...
//!
//! Note: This service uses println!/eprintln! for logging because output is captured by the
//! Windows Service infrastructure and written to service log files.
...
mod windows_service {
    ...
    fn find_server_executable() -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        let candidates = [
            ...
            exe_dir
                .join("binaries")
                .join("zerobyte-server-x86_64-pc-windows-msvc.exe"),
    ...
    fn start_server_process(server_exe: &Path) -> Result<Child, Box<dyn std::error::Error>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => {
            windows_service::run().map_err(Box::<dyn std::error::Error>::from)?;
        }
    }
