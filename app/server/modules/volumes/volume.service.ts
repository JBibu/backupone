...
import { createVolumeBackend } from "~/server/modules/backends/backend";
import { cryptoUtils } from "~/server/utils/crypto";
import { toMessage } from "~/server/utils/errors";
import { isNodeJSErrnoException } from "~/server/utils/fs";
import { generateShortId } from "~/server/utils/id";
import { logger } from "~/server/utils/logger";
import { getStatFs, type StatFs } from "~/server/utils/mountinfo";
...
const listVolumes = async () => {
	...
	return db.query.volumesTable.findMany({
...
const findVolume = async (idOrShortId: string | number) => {
	...
	return db.query.volumesTable.findFirst({
		where: {
...
const updateVolume = async (idOrShortId: string | number, volumeData: UpdateVolumeBody) => {
	...
	const configChanged =
		volumeData.config !== undefined && JSON.stringify(existing.config) !== JSON.stringify(volumeData.config);
...
const getFilesystemRoots = async (): Promise<{ roots: string[] }> => {
	if (process.platform === "win32") {
		...
		return { roots: roots.length > 0 ? roots : ["C:\\"] };
	}
	return { roots: ["/"] };
};
