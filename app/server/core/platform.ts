...
export const IS_TAURI =
	Boolean(process.env.TAURI) || ((IS_WINDOWS || IS_MACOS) && process.env.NODE_ENV === "production");
...
export function getDefaultPath(): string {
	...
	if (IS_WINDOWS) {
		...
		const paths = [execDir, process.cwd(), process.env.PATH || "", path.join(systemRoot, "System32"), systemRoot];
